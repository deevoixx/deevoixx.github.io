...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <head>
        ...
        <link href="CSS/recordPage%20css.css?v=3" rel="stylesheet"/>
    </head>
    <body>
    <form>
        ...
        <div class="quiz-sec m-2">
            ...
            %>
                <div class='category'>
                    <%= category.language %><hr>
                    <div class='quizTopicSec'>
                        <%
                            foreach (dynamic record in actions.Where(x => x.language == category.language))
                            {
                        %>
                            <div onclick="setModal('<%= Base64Encode(record.language) %>', '<%= Base64Encode($"## <u>{record.title}</u><br><br>\n ##### {Session["firstname"].ToString()} {Session["lastname"].ToString()} \n ### Your Highest Score is {record.score} out of {record.items}\n###### Date Taken: {record.date}") %> ')" class='p-2 quizTopic' data-bs-toggle='modal' data-bs-target='#lesson-modal'>
